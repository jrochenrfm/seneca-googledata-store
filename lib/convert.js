...
/**
 * Create a Google Datastore list from a Javascript list.
 * Google Datastore does not store lists of lists.
 *
 * params
 * list - A list object
 */
function createListValue(list){
  var result = [];
  for(var i = 0; i < list.length; i++){
    result[i] = exports.createValueObject(list[i]);
  }
  return list;
}
...
function createField(valueOb){
  ...
  else if(dbType === 'listValue'){
    console.log('got here');
    for(var j = 0; j < valueOb.listValue.length; j++){
      valueOb.listValue[j] = createField(valueOb.listValue[j]);
    }
    return valueOb.listValue;
  }
